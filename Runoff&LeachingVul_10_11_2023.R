...
# 1 = histosols 
# 2 = non-histosols 
taxcode <- vrt(list.files("~/SOLUS/Inputs/taxcode/", recursive=TRUE, full.names=TRUE), filename="./Inputs/taxcode_SSURGO_10_04_2023.vrt")
taxcode <- rast("./Inputs/taxcode_SSURGO_10_04_2023.vrt")
...
# 2 = perched 
# 3 = not rated 
wtcode <- vrt(list.files("~/SOLUS/Inputs/wtcode/", recursive=TRUE, full.names=TRUE), filename="./Inputs/wtcode_SSURGO_10_04_2023.vrt") 
wtcode <- rast("./Inputs/wtcode_SSURGO_10_04_2023.vrt")
...
# 1 = water table <24" 
# 2 =  water table >24" 
wt24in <- vrt(list.files("~/SOLUS/Inputs/wt_24in_flag/", recursive=TRUE, full.names=TRUE), filename="./Inputs/wt24in_SSURGO_10_04_2023.vrt") 
wt24in <- rast("./Inputs/wt24in_SSURGO_10_04_2023.vrt")
...
# 0.28 < 0.32 = 3
# >= 0.32 = 4
kwfact <- vrt(list.files("~/SOLUS/Inputs/kfactor/", recursive=TRUE, full.names=TRUE), filename="./Inputs/kwcode_SSURGO_10_04_2023.vrt") 
kwfact <- rast("./Inputs/kwcode_SSURGO_10_04_2023.vrt")
plot(kwfact, col=c("#ffffbf","#fee08b","#fc8d59","#d53e4f"))

# hsg 
hsg <- vrt(list.files("~/SOLUS/Outputs/hsgtiles/", recursive=TRUE, full.names=TRUE), filename="./Inputs/hsg_SOLUS_10_04_2023.vrt")
...
vul.inputs <- c(taxcode.resamp, wtcode.resamp, wt24in.adj, kwfact.adj, hsg, slope, fragvol)

names(vul.inputs) <-c("taxcode", "wtcode", "wt24in", "kwfact", "hsg", "slope", "fragvol")

writeRaster(vul.inputs, filename = "./Inputs/vulinputs.tif", overwrite=TRUE)
...
# make tiles 
makeTiles(vul.inputs, x, filename="./Inputs/vultiles/vultile_.tif")
...
# list input tiles 
inputtiles <- list.files("./Inputs/vultiles/", full.names = TRUE)
...
for(i in 1:100){
  ...
  lapp(c(vul.inputs$wt24in, vul.inputs$kwfact, vul.inputs$slope, vul.inputs$hsg, vul.inputs$runoff), 
       fun=runoffvul,
       filename = paste0("./Outputs/runoffvul/runoffvultile_", i, ".tif"),
       overwrite = TRUE,
       wopt = list(datatype = "INT1U"))
}

#vrt
runoffvrt <- vrt(list.files("~/SOLUS/Outputs/runoffvul/", recursive=TRUE, full.names=TRUE), filename="~/SOLUS/Outputs/SOLUS_runoffvul_10_05_2023.vrt")

#plot
plot(runoffvrt, col=viridis::viridis(4))

# mask
runoffvrt.notcom<- terra::mask(runoffvrt, kwfact.adj)

writeRaster(runoffvrt.notcom, filename="./Outputs/SOLUS_srp_10_11_2023.tif")

# plot
plot(runoffvrt.notcom, col=viridis::viridis(4))
...
for(i in 1:100){
  ...
  lapp(c(vul.inputs$taxcode, vul.inputs$wt24in, vul.inputs$hsg, vul.inputs$slope, vul.inputs$kwfact, vul.inputs$fragvol, vul.inputs$leaching), 
       fun=leachingvul,
       filename = paste0("./Outputs/leachingvul/leachingvultile_", i, ".tif"),
       overwrite = TRUE,
       wopt = list(datatype = "INT1U"))
}

# .vrt 
leachingvrt <- vrt(list.files("~/SOLUS/Outputs/leachingvul/", recursive=TRUE, full.names=TRUE), filename="~/SOLUS/Outputs/SOLUS_leachingvul_10_05_2023.vrt", overwrite=TRUE)

#plot
plot(leachingvrt, col=viridis::viridis(4))

writeRaster(leachingvrt, filename="./Outputs/SOLUS_nslp_10_11_2023.tif")
